...
# From Wikipedia:  F_n = F_{n-1} + F_{n-2} 
# base cases
# fib(1) = 1
# fib(2) = 1 
# recursive cases
...
def pal(s)
  ...
  if 
    s[0] == s[-1]
    # indexing into strings, working from both ends
    pal(s[1..-2])
  else 
    return false 
  end
...
def binary_helper(digits, binary, result)
  # counts down on the number of digits represented
  if digits == 0 
    result += binary + " "
  else 
  # for every digit I am given, I can represent it as a 1 or a 0 
...
def travel(x,y)
  # tried this, but didn't get very far. I hope we can talk about it! 
end
...
# # Palindrome Tests
# # homework, stripping off letters from base case 
